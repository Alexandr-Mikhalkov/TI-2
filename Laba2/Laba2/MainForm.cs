...
namespace Laba2
{
    public partial class MainForm : Form
    {
        ...
        private void RegTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            RegLabel.Text = $"Ñîñòîÿíèå ðåãèñòðà (24): {length}";
        }
        ...
        private void ExecuteButton_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Äëèíà ðåãèñòðà äîëæíà áûòü ðàâíà 24 ñèìâîëàì!", "Îøèáêà", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }

            if (string.IsNullOrEmpty(PlainTextBox.Text))
            {
                MessageBox.Show("Âûáåðèòå ôàéë ñ èñõîäíûì òåêñòîì!", "Îøèáêà", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
        ...
        string ArrayToStr(bool[] array)
        {
            ...
            {
                result.Append("Ïåðâûå 10 áàéò: \n");
                for (int i = 0; i < 80; i++)
                {
                    result.Append(array[i] ? '1' : '0');
                }

                result.Append($"{Environment.NewLine}Ïîñëåäíèå 10 áàéò: \n");
